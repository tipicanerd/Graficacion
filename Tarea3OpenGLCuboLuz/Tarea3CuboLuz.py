...
def cubo():
    ...
    for color,cara,normal in zip(colores,caras, normales):
        ...
        for index in cara:
            #glColor4fv(color)
            #Para que la luz no se coma el color, se cambia el material
...
def main():
    pygame.init()
    pygame.display.set_caption('Cubito con Iluminación') #Nombre de la ventana
    display=(600,600)
    ...
    while True:
        ...
        glMaterialfv(GL_FRONT, GL_DIFFUSE,colorl1)
        glRotatef(1, 90.0, 120.0, 30.0)
        cubo()
        pygame.display.flip()
        pygame.time.wait(20)
