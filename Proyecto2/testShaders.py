...
pygame.display.set_caption('Dio en el calabazo afectado por el hamon') #Nombre de la ventana
...
Rpos = np.array([1.0, -0.75, 0.75])

#VERDE
Gcolor = np.array([0.0, 1.0, 0.0])
Gpos = np.array([-1.0, 0.75, 0.75])

#AZUL
Bcolor = np.array([0.0, 0.0, 1.0])
Bpos = np.array([-1.0, -0.75, 0.75])
...
m_view_cuarto = np.column_stack([np.vstack([np.eye(3), [0.0,0.0,-5.0]]), np.array([0.0,0.0,0.0,1.0])])
m_view_mesa = np.column_stack([np.vstack([np.eye(3), [-1.2, -1.,  -5.5]]), np.array([0.0,0.0,0.0,1.0])])
m_view_libro = np.column_stack([np.vstack([np.eye(3), [-1.3, -0.325,  -5.8]]), np.array([0.0,0.0,0.0,1.0])])
m_view_dio = np.array([
...
m_view_dio_chiquito = np.column_stack([np.vstack([np.eye(3), [-1., -0.8,  -5.5]]), np.array([0.0,0.0,0.0,1.0])])

m_view_kono_dio_da = np.column_stack([np.vstack([np.eye(3), [0., 0.,  -5.5]]), np.array([0.0,0.0,0.0,1.0])])
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_PLUS:
                m_view_cuarto[3,2] += 0.1
                m_view_mesa[3,2] += 0.1
                m_view_libro[3,2] += 0.1
                m_view_dio[3,2] += 0.1
                m_view_dio_chiquito[3,2] += 0.1
    ...
    cuarto.draw(m_proj,m_view_cuarto,cam_pos,ambiente, colores_fijos, pos_fijas, color_var, pos_var, lucesOn)
    mesa.draw(m_proj,m_view_mesa,cam_pos,ambiente, colores_fijos, pos_fijas, color_var, pos_var, lucesOn)
    libro.draw(m_proj,m_view_libro, cam_pos, ambiente, colores_fijos, pos_fijas, color_var, pos_var, lucesOn)
    ...
    DIO.draw(m_proj,m_view_dio,cam_pos,ambiente, colores_fijos, pos_fijas, color_var, pos_var, lucesOn)
    DIO_chiquito.draw(idx_tex, m_proj, m_view_dio_chiquito)
    
    if alphaDio:
        konoDioDa.draw(alphaDio,m_proj,m_view_kono_dio_da)
